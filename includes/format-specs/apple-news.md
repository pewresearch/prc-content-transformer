...
### Calls to Action
Apple News has no button role. Render a call to action as a `body` component containing a single hyperlink, with `textStyle: "default-outbound-action"`.

## Allowed Component Roles

Emit only the roles listed below. The compiler drops any other role before validation.

<!-- prc-anf-roles:start -->
<!-- Generated by plugins/prc-apple-news/bin/sync-anf-role-spec.php — do not edit by hand. -->

Capability manifest v1.0.0 (ANF 1.11).
...
2. **Headings** become the corresponding `heading2`-`heading6` role. The article headline is emitted by the compiler, so never emit `title`, `heading1`, `intro`, or `byline`.
