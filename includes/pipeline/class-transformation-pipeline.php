...
class Transformation_Pipeline {
	...
	private static function call_ai(
		...
		// Step 4: Post-process.
		$output = $provider->post_process( $response );

		// Step 5: Validate.
		if ( ! $provider->validate( $output ) ) {
			if ( $attempt < self::MAX_RETRIES ) {
				$retry_prompt = Prompt_Builder::build_retry_prompt(
					$user_prompt,
					$output,
					'Output failed format validation for provider: ' . $provider->get_name()
				);
			...
			return new Transformation_Result(
				...
				0,
				$tokens_estimated,
				'Output failed validation after ' . ( $attempt + 1 ) . ' attempt(s).'
			);
		}
